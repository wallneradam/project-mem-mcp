...
The `last_dream:` frontmatter rewrite is atomic with the `set_project_memory` writeback via the `bump_last_dream=True` MCP parameter (added in 0.4.7). Earlier versions used a separate `update_dream_timestamp.py` Bash call owned by the subagent; that script was removed in 0.4.7. `set_project_memory` also preserves an existing YAML frontmatter when the caller's payload doesn't include one — callers can pass plain markdown without knowing about the frontmatter convention.
