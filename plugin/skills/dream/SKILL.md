...
The consolidator subagent writes back via a series of `update_project_memory` SEARCH/REPLACE patches (one localized edit per call), each with `bump_last_dream=True`, which refreshes the `last_dream:` YAML frontmatter. This incremental writeback replaced the old single whole-file `set_project_memory` rewrite (default since 0.4.19) — see *Important* for why. You do not need a post-step here.
...
- **Writeback is incremental (default since 0.4.19).** The consolidator emits a series of `update_project_memory` SEARCH/REPLACE patches that touch only the regions that change, instead of one whole-file `set_project_memory` rewrite. Why: the dominant cost of a dream is output-token generation; a full rewrite re-emits tens of thousands of tokens of unchanged text, which made a large file (PyneSys's ~128KB MEMORY.md) take 10+ minutes even on Sonnet. `set_project_memory` is now only the fallback for wholesale restructuring or a timestamp-only refresh.
